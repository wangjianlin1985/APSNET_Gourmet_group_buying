...
namespace DAL
{
    ...
    public class dalUserInfo
    {
        ...
        {
            string sql = "insert into UserInfo(user_name,password,name,gender,birthDate,userPhoto,telephone,email,areaObj,address,regTime) values(@user_name,@password,@name,@gender,@birthDate,@userPhoto,@telephone,@email,@areaObj,@address,@regTime)";
            /*构建sql参数*/
            ...
            parm[8].Value = userInfo.areaObj; //所在区域
            parm[9].Value = userInfo.address; //家庭地址
            parm[10].Value = userInfo.regTime; //注册时间
        ...
        public static ENTITY.UserInfo getSomeUserInfo(string user_name)
        {
            ...
            if (DataRead.Read())
            {
                ...
                userInfo.areaObj = Convert.ToInt32(DataRead["areaObj"]);
        ...
        {
            string sql = "update UserInfo set password=@password,name=@name,gender=@gender,birthDate=@birthDate,userPhoto=@userPhoto,telephone=@telephone,email=@email,areaObj=@areaObj,address=@address,regTime=@regTime where user_name=@user_name";
            /*构建sql参数信息*/
            SqlParameter[] parm = new SqlParameter[] {
             new SqlParameter("@password",SqlDbType.VarChar),
             new SqlParameter("@name",SqlDbType.VarChar),
            ...
            parm[7].Value = userInfo.areaObj;
            parm[8].Value = userInfo.address;
            parm[9].Value = userInfo.regTime;
